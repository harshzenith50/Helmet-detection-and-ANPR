...
while cap.isOpened():
    ret, frame = cap.read()  # read([, image]) -> retval(true or false), image
    if ret == True:
                ...
                if inside_box([x1r, y1r, x2r, y2r], [x1h, y1h, x2h, y2h]):  # if this head inside this rider bbox
                    ...
                    if helmet_present[0] == True:  # if helmet present
                        ...
                        frame = cv2.putText(frame, f'{round(helmet_present[1], 1)}', (x1h, y1h + 40),
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
                        try:
                            cv2.imwrite(f'riders_pictures/{time_stamp}.jpg', frame[y1r:y2r, x1r:x2r],[int(cv2.IMWRITE_JPEG_QUALITY), 100])
                        except:
                        ...
                        for num in number_list:
                            ...
                            if inside_box([x1r, y1r, x2r, y2r], [x1_num, y1_num, x2_num, y2_num]):
                                try:
                                    ...
                                    cv2.imwrite(f'number_plates/{time_stamp}_{conf_num}.jpg', num_img,[int(cv2.IMWRITE_JPEG_QUALITY), 100])
...
a = []
path = "C:\\Users\\Harsh\\motorcycle_license_plate-main\\number_plates"
...
for filename in os.listdir(path):
   with open(os.path.join(path, filename), 'rb') as f:
    response = requests.post(
        ...
        files=dict(upload=f),
        headers={'Authorization': 'Token 4bd7132459fe03bf5cf13eb1f0d58dd26f76c4bf'})
    r = response.json()
#print(r)
#print("License Plate: ")
    a.append(r['results'][0]['plate'])
    time.sleep(1.02)
for i in set(a):
